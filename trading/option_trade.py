...
from ctrl.visual_market import OpVMarket
...
class OptionOrder:
	hand2gu=10000
	uid=0
	str_title='id order_time option_code optype price amount service_charge pay'
	def __init__(self, orderTime, optionCode, optype, curPrice, amount, serviceCharge):
		self.id=OptionOrder.uid
		OptionOrder.uid+=1
		self.orderTime=orderTime       #order time
		self.optionCode=optionCode      #which option
		self.optype=optype             #operation type: 0 for buy, 1 for sell, 2 for pingcang
		self.curPrice=curPrice         #current option price
		self.amount=amount             #number of shou option
		self.serviceCharge=serviceCharge    #fu wu fei
		self.totalCost=0.0        #total cost
		if(self.optype==0): self.totalCost=self.amount*self.hand2gu*self.curPrice+self.serviceCharge
		elif(self.optype==1): self.totalCost=-self.amount*self.hand2gu*self.curPrice+self.serviceCharge
...
class OptionTrade:
	def __init__(self, decision, tradeinterval=[1, 'd'], curtime=None, sim=True, sim_st=None,):
		...
		self.market=OpVMarket(self.interval, self.curtime)              #trade market
		if self.curtime==None: self.curtime=market.curtime
		self.orders=[]                     #option orders
		self.oldorders=[]
		self.decision=decision             #making decision
		self.serviceCharge=6.0
	...
	def periodProfit(self):
		...
		self.recorder.addPrice(self.etf.getLastPrice())
		self.inmoneyp=0.0
	...
	def updateFromMarket(self):
		...
		for op in mops:
			...
			else: self.options.append(op)	

	def getOptionFCode(self, code):
