	...
	def __init__(self, is_train, params):
		...
		for i in range(self.num_scopes):
			with tf.variable_scope("lstm_scope"+str(i)):
				...
				print(cell.name)
				if i==0: initstate=[LSTMStateTuple(tf.zeros([self.batch_size, self.lstm_size], tf.float32), 
					tf.zeros([self.batch_size, self.lstm_size], tf.float32))]
				else: initstate=[LSTMStateTuple(tf.zeros([self.batch_size, self.lstm_size], tf.float32), 
					self.in_outputs[i-1])]
				for j in range(self.num_layers-1): initstate.append(LSTMStateTuple(tf.zeros([self.batch_size, self.lstm_size], tf.float32), 
					tf.zeros([self.batch_size, self.lstm_size], tf.float32)))
				...
				if i>0: re_outs=tf.add(0.8*re_outs, 0.2*self.in_outputs[i-1])
