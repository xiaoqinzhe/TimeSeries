...
class ResidualRnn:
	def __init__(self, is_train, params):
		...
		with tf.variable_scope('scopes_attention'):
			# attention
			Wc = tf.get_variable('weighted_c', shape=(self.lstm_size, 1))
			Wh = tf.get_variable('weighted_h', shape=(self.lstm_size, 1))
			ms = [tf.nn.tanh(tf.matmul(self.states[i][-1].c, Wc) + tf.matmul(self.in_outputs[i], Wh)) for i in range(self.num_scopes)]
			ws = tf.nn.softmax(ms)
			re_outs = tf.reduce_sum(tf.multiply(ws, self.in_outputs), axis=0)
		self.union_output=re_outs
		#print(self.union_output)
		...
		#print(w_o.name)
		self.outputs=tf.matmul(self.union_output, w_o)+b_o
		#loss and optimizer
